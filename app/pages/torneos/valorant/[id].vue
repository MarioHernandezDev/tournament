<template>
  <div class="max-w-[1380px] mx-auto px-4 sm:px-6 py-10 space-y-10">
    <NuxtLink to="/torneos/valorant" class="inline-flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors">
      <UIcon name="i-heroicons-arrow-left" class="w-4 h-4" />
      Volver a Torneos de Valorant
    </NuxtLink>
    ...
    <div v-if="tournament" class="space-y-14">
      ...
      <section class="relative overflow-hidden rounded-3xl border border-zinc-800/80 bg-zinc-950">
        ...
        <div class="relative z-10 px-6 sm:px-10 pb-8 -mt-16 sm:-mt-20 space-y-4">
          ...
          >
            <UIcon name="i-simple-icons-valorant" class="w-3.5 h-3.5" />
      ...
      <TournamentsBracketView :tournament-id="tournament.id" game="valorant" />
      <TournamentsTournamentScoreboard :tournament-id="tournament.id" game="valorant" />
    </div>
...
const tournament = computed(() => tournaments.value.find((t) => t.id === route.params.id && t.game === 'valorant'))

const gameConfig = useGameConfig('valorant')
